...
var ChannelService = function () {
    ...
    ChannelService.prototype.getBy = function (url, id) {
        return this._http.get(url + id).map(function (response) {
            return response.json();
        }).catch(this.handleError);
    };
    ChannelService.prototype.getBriefBy = function (id) {
        return this._http.get('api/channel/' + id).map(function (response) {
            return response.json();
        }).map(function (x) {
    ...
    ChannelService.prototype.put = function (url, obj) {
        ...
        return this._http.put(url, body, options).map(function (response) {
            if (response.ok) return Observable_1.Observable.empty();else throw new Error('Http request has failed. Status: ' + response.status + ' - ' + response.statusText);
        }).catch(this.handleError);
    };
...
ChannelService = __decorate([core_1.Injectable(), __metadata("design:paramtypes", [http_1.Http])], ChannelService);
