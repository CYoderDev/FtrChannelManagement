...
var ChannelLogoService = function () {
    ...
    ChannelLogoService.prototype.putBody = function (url, obj) {
        ...
        var ret = this.openLocalImage(obj, function (val) {
            _this._http.put(url, val, new http_1.RequestOptions({ headers: headers, withCredentials: true })).map(function (response) {
                if (response.ok) return Observable_1.Observable.empty;else throw new Error('Http PUT request failed. Status: ' + response.status + ' - ' + response.statusText);
            }).catch(_this.handleError);
        });
        return ret.readAsDataURL(obj);
    };
    ChannelLogoService.prototype.put = function (url) {
        return this._http.put(url, null, new http_1.RequestOptions({ withCredentials: true })).map(function (response) {
            if (response.ok) return Observable_1.Observable.empty;else throw new Error('Http PUT request failed. Status: ' + response.status + ' - ' + response.statusText);
        }).catch(this.handleError);
    };
    ChannelLogoService.prototype.post = function (url, obj) {
        var headers = new http_1.Headers({ 'Content-Type': 'image/png' });
        return this._http.post(url, obj, new http_1.RequestOptions({ headers: headers, withCredentials: true })).map(function (response) {
            if (response.ok) return Observable_1.Observable.empty;else throw new Error('Http POST request failed. Status: ' + response.status + ' - ' + response.statusText);
        });
    };
    ChannelLogoService.prototype.performRequest = function (endPoint, method, body, contentType, uploadContentType) {
        ...
        if (uploadContentType === void 0) {
            uploadContentType = null;
        }
        var headers = new http_1.Headers({ 'Content-Type': contentType });
        var options = new http_1.RequestOptions({ headers: headers });
        if (body) options.body = body;
        if (uploadContentType) headers.append('Upload-Content-Type', uploadContentType);
        if (method == 'GET') {
            headers.append('Cache-Control', 'no-cache,no-store');
            return this._http.get(endPoint, options).map(this.extractData).catch(this.handleError);
        } else if (method == 'PUT') {
            options.withCredentials = true;
            return this._http.put(endPoint, body, options).map(this.extractData).catch(this.handleError);
        } else {
            options.withCredentials = true;
            return this._http.post(endPoint, body, options).map(this.extractData).catch(this.handleError);
        }
    };
    ...
    ChannelLogoService.prototype.extractData = function (response) {
        ...
        if (contentType) {
            if (contentType.startsWith('image')) return response.text();
        }
        return response.json();
    };
...
ChannelLogoService = __decorate([core_1.Injectable(), __metadata("design:paramtypes", [http_1.Http])], ChannelLogoService);
